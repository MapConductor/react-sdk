...
export interface KMLFeatureFingerPrint {
    id: number;
    geometry: number;
    properties: number;
    style: number;
...
function fpEquals(a: KMLFeatureFingerPrint, b: KMLFeatureFingerPrint): boolean {
    return a.id === b.id &&
...
function buildDefaultId(geometry: KMLGeometry, properties: Record<string, unknown>): string {
    return ((objectHash(geometry) * 31 + objectHash(properties)) | 0).toString();
}

export class KMLFeatureState {
    readonly id: string;

    private _geometry: KMLGeometry;
    ...
    private readonly subject = createSubject<KMLFeatureFingerPrint>(fpEquals);
    ...
    get geometry(): KMLGeometry { return this._geometry; }
    set geometry(v: KMLGeometry) { this._geometry = v; this.subject.next(this.fingerPrint()); }
    ...
    asObservable(): { subscribe: (fn: (fp: KMLFeatureFingerPrint) => void) => () => void } {
