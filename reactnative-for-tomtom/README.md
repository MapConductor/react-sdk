...
このパッケージは **動く最小のブリッジ** です。読んで、コピーして、`TomTom` を
あなたの地図SDK名に置き換えるところから始めてください。

`react-for-tomtom` が「web の**ドライバー**」の雛形なのに対し、こちらは
「既にあるネイティブドライバーを **React Native へ出す**」ための雛形です。
つまり **`android-for-<sdk>` と `ios-for-<sdk>` が先に必要** で、このパッケージは
その 2 つを RN のブリッジに載せるだけです。

android-sdk の `android-for-tomtom`、ios-sdk の `ios-for-tomtom`、
react-sdk の `react-for-tomtom` と**同じ構造**にしてあります。
...
| # | 実装点 | ファイル | 行数の目安 |
|---|---|---|---|
| A | Android のアダプタ | `android/.../TomTomMapViewWrapper.kt` | 約 95 |
| B | iOS のアダプタ | `ios/TomTomReactNativeView.swift` | 約 90 |
| C | JS の薄いラッパー | `src/*.ts` | 約 120 |
| D | ObjC の ViewManager | `ios/MapConductor*ViewManager.m` | 約 25 |
| E | Android の ViewManager / Package | `android/.../MapConductorTomTom*.kt` | 約 95 |
...
1. `reactnative-for-tomtom` をコピーして `reactnative-for-<sdk>` にする
2. `TomTom` を SDK 名へ一括置換（`REACT_CLASS` / `RCT_EXPORT_MODULE` /
...
```bash
# JS
npm run build -w @mapconductor/reactnative-for-tomtom
...
`android-for-tomtom` / `ios-for-tomtom` の地図は本物の SDK ではなく、
